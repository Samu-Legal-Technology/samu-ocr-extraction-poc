...
export class TextExtractor {
    ...
    async syncExtract(bucket: string, key: string): Promise<TextExtractorSyncResult> {
        ...
        const extractedLines = extraction.Blocks
    ...
    async asyncExtract(bucket: string, key: string): Promise<TextExtractorAsyncResult> {
        if (!this.notify.roleArn)
            throw Error('Missing notify roleArn')

        if (!this.notify.topicArn)
            throw Error('Missing notify topicArn')
    ...
    async fetchJobOutputFrom(result: TextExtractorAsyncResult): Promise<string[]> {
        if (!result.jobId) {
        ...
        do {
          ...
          const extraction: GetDocumentTextDetectionCommandOutput = await textract.send(
